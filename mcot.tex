...
\section*{Motivation}
Etant sensibiliser à la cybersécurité, j'ai constaté que le chiffrement de flux audio est souvent limité par la bande passante et la latence. Je souhaite relever ce défi en étudiant comment la réduction d'entropie permet d'intégrer un chiffrement robuste en altérant le moins la fluidité d'une communication en temps réel.
...
\section*{Positionnement thématique}
 INFORMATIQUE (Informatique pratique), INFORMATIQUE (Informatique théorique), PHYSIQUE (Physique Ondulatoire).
...
Pour sécuriser une communication vocale en temps réel, l'on doit d'abord chercher une certaine efficacité dans sa protection mais aussi garantir une fluidité de l'échange. Cette problématique s'articule autour de deux axes de recherche successifs, la réduction de l'entropie de la source par compression, puis l'application d'un système de secret robuste par chiffrement.

Une source audio continue présente une redondance élevée que la théorie de Shannon cherche à minimiser pour atteindre un débit proche de l'entropie de la source [1]. Si le codage statistique (codage de Huffman) permet une réduction sans perte, l'introduction de la Transformée de Fourier Rapide (FFT) permet d'exploiter la redondance spectrale du signal [2]. En passant dans le domaine fréquentiel, il devient possible d'appliquer des seuils de quantification basés sur des critères de fidélité psychoacoustiques [1], et ainsi réduire la puissance d'entropie du flux audio, augmentant ainsi la "distance d'unicité" lors de la phase ultérieure de chiffrement [3].
...
La défi majeur de cette chaîne de traitement (compression + chiffrement) réside dans le respect de la continuité temporelle. La recommandation ITU-T G.114 fixe une limite de latence de bout en bout de 150 ms pour garantir une clareté de communication "acceptable" [4]. Ce délai total ($T_{total}$) est la somme des temps de captation, de traitement ($T_{traitement}$), de réseau et de restitution [4].$$T_{total} = T_{captation} + T_{traitement} + T_{réseau} + T_{restitution}$$La littérature souligne que la complexité algorithmique, notamment lors des calculs de FFT ou de transformations de clés, est la principale source de gigue (jitter), piques, au niveau de $T_{traitement}$ [2]. Cette instabilité temporelle, variable selon l'environnement utilisé (système d'exploitation), menace directement le respect de la limite des 150 ms [4]. La recherche de solutions existantes conduit à une incompatibilité fréquente entre la compression très poussée, qui est gourmande en CPU, et un maintien d'une latence faible de communication [3]. 
...
\section*{Objectifs}
L'objectif est de concevoir une chaîne de communication vocale sécurisée (compression + chiffrement) respectant la limite de 150 ms de latence de la norme ITU-T G.114. Pour cela, je vais :
\begin{enumerate}
